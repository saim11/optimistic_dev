...
class cordinator extends CI_Controller {
    ...
    public $ws;
    public $tblName 		= 	"co_ordinator";
    public $pKey			=	"";
    public $colPrefix       =   "co_";
    public $moduleName 		=	"Co-ordinator";
    public $controller 		=	"cordinator";
    public $per_page 		=	"10";
    public $tStatus 		=	"";
    public $listView 		=	"cordinator";
    public $editView = "editCordinator";
	public $model = array();                                 
    ...
    public function __construct(){
        ...
        $this->model  =   array(                                
            ...
            ,'view_update' => true
            ,'view_list' => true
            ,'view_name' => 'Coordinator Code'
            ,'type' => 'text'
            ,'required' => true
            , 'list'=>array()
            )
            ,array('name' => 'co_suburb'
            ,'view_update' => true
            ,'view_list' => true
            ,'view_name' => 'Coordinator Suburb'
            ,'type' => 'text'
            ,'required' => true
            , 'list'=>array()
            )
            ...
            ,array('name' => 'count'
               ...
               ,'required' => true
               ,'icon' => 'entypo-docs'
               , 'query'=>"SELECT COUNT(`answer_id`) AS 'count' FROM `answer` WHERE `answer_co`='@value@'"
               , 'detail_link'=>'manage/answer?answer_co=@value@'
               )
            // ,array('name' => 'co_type'
            // ,'view_update' => true
            // ,'view_list' => false
    ...
    public function control($type="",$editID="")
    {
        ...
        $data['page_title'] = (($type=="") ? "Coordinator" : ucfirst($type))." ".rtrim($this->moduleName."/Coordinator",'s');
        //CKEditor
        ...
        $this->load->view('admin/'.$this->editView, $data);
        $this->load->view('admin/footer');
	}
            ...
            if($cols['view_update'])
            {
                ...
                if($cols['type'] != 'image' && $cols['type'] != 'file' && $cols['type'] != 'checkbox' && $cols['type'] != 'custom_query' && $cols['type'] != 'action' && $cols['type'] != 'detail' )
                {
                    if(isset($_POST[$cols['name']]))
                    {
                        ...
                        if($cols['name'] == "co_password" && $old_password['co_password'] != $this->input->post($cols['name']))
                        {
    ...
    public function editRecord($type="",$editID="")
    {
        
        $data = array();
        // $data[$this->colPrefix.'added']=date('Y-m-d H:i:s');
        // $data[$this->colPrefix.'updated']=date('Y-m-d H:i:s');
        $old_password = $this->SqlModel->getSingleRecord("co_ordinator", array("co_id"=>$editID));
        // 
        foreach ($this->model as $cols) {
