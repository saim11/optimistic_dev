...
class Home extends CI_Controller {
	...
	public function page($page_uri="")
	{
		...
		if($pageData["page_id"] > 17 || $pageData["page_id"] == 8 || $pageData["page_id"] == 9 || $pageData["page_id"] == 10 || $pageData["page_id"] == 11 || $pageData["page_id"] == 12 || $pageData["page_id"] == 13)
		{
			$showView = 'page';
		}
	 ...
	 public function login()
    {
        ...
        else
        {
            ...
            if($user_details != 0)
            {
                if($user_details['user_is_verified'] == "No")
                {
                    $this->session->set_userdata('error_from_php', 'Whoops! Please first go to your Inbox (also check span as well) and follow the procedure to activate your account.');
                    redirect("login.html");
                }
                else
                {
                    ...
                    if($this->session->userdata('redirect_url'))
                    {
                        ...
                        redirect('application-form.html');
                    }
                    else
                    {
                        redirect('/');
    ...
    public function applicationform()
    {
        if(!$this->session->userdata('user_id'))
        {
            ...
            redirect('login.html');
                ...
                $this->load->view('header', $dataheader);
                $this->load->view('applicationform-list');
                $this->load->view('footer');
    }
