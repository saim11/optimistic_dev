	...
	function submitForm() {
		...
		studentUsiData = new Object();
					...
					$('.input-field-box').keyup(function () {
						...
						}else if($("#box10").is(':checked')){
							$('#box10').attr('checked', true);
							$('#box9').attr('checked', false);
							chkOffice_use_1Selected = $("#box10").val();
						}else if($('#box9').is(':checked') && $("#box10").is(':checked')) {
						...
						}else if($("#box12").is(':checked')){
							$('#box11').attr('checked', false);
							chkOffice_use_2Selected = $("#box12").val();
						}else if($("#box12").is(':checked') && $("#box11").is(':checked')) {
						...
						}else if($("#box14").is(':checked')){
							$('#box13').attr('checked', false);
							chkOffice_use_3Selected = $("#box14").val();
						}else if($("#box14").is(':checked') && $("#box13").is(':checked')) {
		...
		$.ajax({
			type: 'POST',
			data: {studentUsiData: studentUsiData, user_id: <?php echo $user_id ?> , form_id: <?php echo $form_id ?>  },
			url: '<?php echo site_url('application_forms_update/form_apply1_submit_data') ?>',
			success: function(result){
				// ======================
				$("#Approved").html('<div class="alert panel-done" ><div class="panel-body">Student Form has been Approved..!</div></div>');
				window.location.href = '<?php echo base_url()."manage/studentForms/control/forms/".$user_id?>';
				// window.location.replace('<?php echo base_url("application-form.html") ?>');
				// $.ajax({
				// 	type: 'POST',
				// 	data: {form_submitted: "Submitted"},
				// 	url: '<?php echo site_url('application_forms_update/form_apply1_submitted')?>',
				// 	success: function(result){
...
<div id="Approved">
		...
		<div class="panel panel-success row Add_details">
			<h3 class="form-heading">USI Permission and Verification Form</h3>
		</div>
	<section class="Welcome_Classic Optim-Form0s row Add_details ">
		<!-- <div >
			<ol class="breadcrumb">
				...
				<li>
					<a href="#">USI Permission and Verification Form</a>
				</li>
			</ol>
		</div> -->
	</section>
	...
	<section  style="background-color: white;">
		<div  style="background-color: white;">
			<div id="divtoconvert" class="row Add_details" style="background-color: white; width: 850px; margin: 0 auto;padding-top: 0px;padding-bottom:20px;">
						...
						<p>You have been advised, agree, understand and consent that the personal information you provide in connection with your
							...
							<div class="col-sm-4">
								<div class="" style="position: relative;">
									<label class="sign-pti" style="position: absolute; left: 22%; top: 7px; z-index: 1;">Signature</label>
									<input disabled="" type="text" id="" style="position: absolute; left: 14%; width: 121px;" class="form-control formpti1 required">
								</div>
							</div>

							<div class="col-sm-4">
								<div class="form-group">
									<label class="sign-pti" style="margin-left: 93%;top: 7px;">Date</label>
									<input disabled="" type="text" id="" style="position: absolute;left: 80%;" class="form-control formpti1 required">
								</div>
							</div>
			...
			<div class="container">
					<div class="row Add_details">
						<div class="col-md-12 col-sm-12">
							<a class="btn btn-default Submit_Button" type="button" id='download' onclick="form_attestation(<?php echo $form_apply1['enrol']['Id']; ?>,<?php echo $form_apply1['enrol']['user_id']; ?>,this.id, '<?php echo $form_apply1['enrol']['submission_date']; ?>'); ello();submitForm()">SUBMIT</a>
							<a class="btn btn-default Approve_Button" href="#Approved" id="approve" onclick="form_attestation(<?php echo $form_apply1['enrol']['Id']; ?>,<?php echo $form_apply1['enrol']['user_id']; ?>,this.id,'<?php echo $form_apply1['enrol']['submission_date']; ?>');submitForm()" type="button">APPROVE</a>
...
<script type="text/javascript">
	...
	<script src='<?php echo base_url(); ?>ofpdfs/f1/form_attested.js'></script>
