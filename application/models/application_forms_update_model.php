<?php
    class application_forms_update_model extends CI_Model {
        ...
        public function get_form_apply1_admin($user="")
        {
            $user_login="";
            $loginUserId = $this->session->userdata('user_id');
            if($user != ""){$user_login=$user;}else{$user_login=$loginUserId;}
            $query = $this->db->get_where('usi_permission_verification_form', array('Id' => $user_login));
        ...
        public function form_apply1($studentUsiData="",$user_id="",$form_id="")
        {
            $loginUserId = $user_id;
            print_r($studentUsiData);
            $query = $this->db->get_where('usi_permission_verification_form', array('user_id' => $user_id,'Id'=>$form_id ,'form_submitted' => 'Submitted'));
            if ($query->num_rows() <= 0 ){
                ...
                //     'contact' => $studentUsiData['mobile_no'],
                //     'email' => $studentUsiData['email_address'],
                //     'usi' => $studentUsiData['usi'],
                //     'identification_usi_application' => $studentUsiData['identification_usi_application'],
                //     'student_name' => $studentUsiData['studentName'],
                ...
                $updateData = array(
                    ...
                    'office_use_2' => $studentUsiData['office_use_2'],
                    'office_use_3' => $studentUsiData['office_use_3'],
                    'form_approval'=>"Approved"
                );
                // print_r(" -in else ".$updateData);
                // update current data
                $this->db->where(array('user_id' => $user_id,'Id'=>$form_id));
                $this->db->where('form_submitted', 'Submitted');
                $this->db->update('usi_permission_verification_form', $updateData);       
                }
            } 

             //  form 1 submit
            public function form_apply1_submitted($form_submitted)
            {
                $loginUserId = $this->session->userdata('user_id');
        ...
        public function get_form_apply1_for_student($user="")
        {
            $user_login="";
            $loginUserId = $this->session->userdata('user_id');
            if($user != ""){$user_login=$user;}else{$user_login=$loginUserId;}
            // $query = $this->db->get_where('enrollment_booklet', array('user_id' => $user));
            $where = array('user_id' => $user);
            $this->db->select('*');
		    $this->db->from('usi_permission_verification_form');
                if(!empty($where))
                {
                $this->db->where($where);	
                }
        ...
        public function get_form_apply2_for_student($user="")
        {
            $user_login="";
            $loginUserId = $this->session->userdata('user_id');
            if($user != ""){$user_login=$user;}else{$user_login=$loginUserId;}
            ...
            $where = array('user_id' => $user);
            $this->db->select('*');
		    $this->db->from('enrollment_booklet');
                if(!empty($where))
                {
                $this->db->where($where);	
                }
        ...
        public function get_form_apply2($editId="")
        {
        ...
        public function form_apply2_studentName($studentName="",$user_id="",$form_id="")
        {   
            $loginUserId = $this->session->userdata('user_id');
            $query = $this->db->get_where('enrollment_booklet', array('user_id' => $user_id));
        ...
        public function form_apply2_qualification($selectedQualification,$user_id="",$form_id="")
        {
            // print_r("selected qual".$selectedQualification);
            $loginUserId = $this->session->userdata('user_id');
            $query = $this->db->get_where('enrollment_booklet', array('user_id' => $user_id,"Id"=>$form_id));
        ...
        public function form_apply2_attachments($selectedAttachments,$user_id="",$form_id="")
        {
        ...
        // form 3 start ============================================
        // get data =================
        public function get_form_apply3($editId="")
        {
            $loginUserId = $this->session->userdata('user_id');
            $query = $this->db->get_where('skills_first_program_enrolment_agreement_for', array('id' => $editId));
            $row1 = $query->row_array();
        ...
        // get data =================
        public function get_form_apply3_for_student($editId="")
        {
            $loginUserId = $this->session->userdata('user_id');
            // $query = $this->db->get_where('skills_first_program_enrolment_agreement_for', array('user_id' => $editId));
            $where = array('user_id' => $editId);
            $this->db->select('*');
		    $this->db->from('skills_first_program_enrolment_agreement_for');
                if(!empty($where))
                {
                $this->db->where($where);	
                }
        ...
        public function form_apply3($enrolment_courseSelected, $mode_of_studySelected, 
                                    ...
                                    $areasOfDiabilitySelected,$user_id="",$form_id="") {

            $loginUserId = $this->session->userdata('user_id');
            $query = $this->db->get_where('skills_first_program_enrolment_agreement_for',array('user_id' => $user_id,"id"=>$form_id));
            print_r($query->row_array());
            // $row = $query->row_array();
        ...
        public function form_apply3_2($textBoxData,$user_id="",$form_id="")
        {
        ...
        //get form 4 data
        public function get_form_apply4($editId)
        {
            $loginUserId = $this->session->userdata('user_id');
            $query = $this->db->get_where('student_induction_form', array('id' => $editId));
            $row1 = $query->row_array();
        ...
        //get form 4 data
        public function get_form_apply4_for_student($editId)
        {
            $loginUserId = $this->session->userdata('user_id');
            // $query = $this->db->get_where('student_induction_form', array('user_id' => $editId));
            $where = array('user_id' => $editId);
            $this->db->select('*');
		    $this->db->from('student_induction_form');
                if(!empty($where))
                {
                $this->db->where($where);	
                }
        ...
        public function form_apply4($form_apply4_data,$user_id="",$form_id="")
        {
            $loginUserId = $this->session->userdata('user_id');
            $query = $this->db->get_where('student_induction_form', array('user_id' => $user_id,"id"=>$form_id));
        ...
        public function form_apply4_2($form_apply4_data_2,$user_id="",$form_id="")
        {
