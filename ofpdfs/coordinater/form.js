
var IconTick = 'data: image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABcAAAATCAYAAAB7u5a2AAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsMAAA7DAcdvqGQAAADpSURBVDhP3ZQtEoUgFIXZhct4kcgSiC8aXYJLIBpdgpFoJBqJRiORSDxP9M2IM/Izzli8M1A8891zjwDBg0UeZOOtcKOhlLpes80mGo / FSNRcQEbhBnYSoLSFivSJws3A0U5xcxuYEBBSgfXzpTAJFxvcYdHLuh91Aq + i8FvYJQN30B1D5d39IaVg3yTr3IXjf / kRxdrMupjn3X8W7kWHW58xAd2m0Og + AonfUgYPG + xgXxMEuQm3Y4NamtMpcDaIwY1o + ICz4nxoEtd / gawpKGNgF4tSjl7fzDx7 / QoEb324CkZPSR6N5QdImVqfbkGFCgAAAABJRU5ErkJggg ==';
var IconBlank = 'data:image/png;base64,';
var img = 'data:image/png;base64,';
var box1, box2, box3, box4, box5;
box1 = box2 = box3 = box4 = box5 = IconBlank;
var co_name=co_email=co_suburb=co_code = "";
const monthNames = ["January", "February", "March", "April", "May", "June",
  "July", "August", "September", "October", "November", "December"
];



let printCoordinaterReport = (data) => {

    header = [
        { text: 'Student ID', bold: true, alignment: 'center', fillColor: '#bfbfbf', style: "header" }
        , { text: 'Student', bold: true, fillColor: '#bfbfbf', style: "header" }
        , { text: 'Stream', bold: true, fillColor: '#bfbfbf', style: "header" }
        , { text: 'Course', bold: true, fillColor: '#bfbfbf', style: "header" }
        , { text: 'Unit Code', bold: true, fillColor: '#bfbfbf', style: "header" }
        , { text: 'Status', bold: true, fillColor: '#bfbfbf', style: "header" }
        , { text: 'Hours', bold: true, fillColor: '#bfbfbf', style: "header" }
        , { text: 'Date', bold: true, fillColor: '#bfbfbf', style: "header" }
        , { text: 'Coordinter', bold: true, fillColor: '#bfbfbf', style: "header" }
    ];
    students = [];
    students.push(header);
    data = JSON.parse(data);
    if(data[0].student_id !== undefined){

        data.map(cols => {
            dataVal = [
            { text: cols.student_id, style: "header", alignment: 'center', }
            , { text: cols.user_name, style: "header" }
            , { text: cols.course_name, style: "header" }
            , { text: cols.course_title, style: "header" }
            , { text: cols.unit_name, style: "header" }
            , { text: cols.unit_status, style: "header" }
            , { text: cols.unit_hours, style: "header", alignment: 'center', }
            , { text: cols.completion_date, style: "header" }
            , { text: cols.co_name, style: "header" }
        ];
        students.push(dataVal);
        co_name = cols.co_name;
        co_email  =cols.co_email;
        co_suburb = cols.co_suburb;
        co_code = monthNames[new Date(cols.completion_date).getMonth()];
        
    });
}
else{
    co_name = data[0].co_name;
    co_email  =data[0].co_email;
    co_suburb = data[0].co_suburb;
    co_code = data[1];
    dataVal = [
        { text: 'NULL', style: "header", alignment: 'center', }
        , { text: 'NULL', style: "header" }
        , { text: 'NULL', style: "header" }
        , { text: 'NULL', style: "header" }
        , { text: 'NULL', style: "header" }
        , { text: 'NULL', style: "header" }
        , { text: 'NULL', style: "header", alignment: 'center', }
        , { text: 'NULL', style: "header" }
        , { text: 'NULL', style: "header" }
        
    ];
    students.push(dataVal);
}
    

console.log(data);






    var docDefinition = {
        margin: [0, 0, 0, 0],
        content:
            [
                {
                    table: {
                        widths: [100, 320],
                        body: [
                            [
                                {
                                    image: img,
                                    width: 150,
                                    height: 65,
                                    alignment: 'center'
                                },
                                {
                                    text: 'OPTIMISTIC FUTURE \nCO-ORDNATOR REPORT', bold: true, fontSize: 12, alignment: "center", style: "text", margin: [0, 30, 0, 0]
                                },
                            ]
                        ]
                    },
                    layout: {
                        paddingTop: function (i, node) { return 0; },
                        paddingBottom: function (i, node) { return 0; },
                        hLineColor: '#fff',
                        vLineColor: '#fff',
                        hLineColor: '#fff',
                        vLineColor: '#fff',
                    }
                }, "\n", "\n",
                {
                    table: {
                        style: "header",
                        widths: [90, 110],
                        body: [
                            [
                                {
                                    text: 'Coordinater Name:',
                                    style: "paragraph",
                                    alignment: 'center',
                                    border: [false, false, false, false]
                                },
                                {
                                    text: co_name,
                                    style: "subParagraph",
                                    bold: true,                                    
                                    border: [false, false, false, false]
                                },
                            ],
                        ]
                    },
                },
                "\n",
                {
                    table: {
                        style: "header",
                        widths: [90, 150],
                        body: [
                            [
                                {
                                    text: 'Coordinater Email:',
                                    style: "paragraph",
                                    alignment: 'center',
                                    border: [false, false, false, false]
                                },
                                {
                                    text: co_email,
                                    style: "subParagraph",
                                    
                                    border: [false, false, false, false]
                                },
                            ],
                        ]
                    },
                },
                "\n",
                {
                    table: {
                        style: "header",
                        widths: [90, 150],
                        body: [
                            [
                                {
                                    text: 'Coordinater Suburb:  ',
                                    style: "paragraph",
                                    alignment: 'center',
                                    border: [false, false, false, false]
                                },
                                {
                                    text: co_suburb,
                                    style: "subParagraph",
                                    
                                    border: [false, false, false, false]
                                }
                            ]
                        ]
                    },
                },"\n",
                {
                    table: {
                        style: "paragraph",
                        widths: [90, 150],
                        body: [
                            [
                                {
                                    text: 'Month :  ',
                                    style: "paragraph",
                                    alignment: 'center',
                                    border: [false, false, false, false]
                                },
                                {
                                    text: co_code,
                                    style: "subParagraph",
                                    border: [false, false, false, false]
                                }
                            ]
                        ]
                    },
                },


                "\n\n",
                {
                    table: {
                        widths: [500],
                        body: [
                            [
                                {
                                    text: 'Students Detail', fillColor: "#0080ff", color: 'white', alignment: 'center', bold: true, style: "textSmall",
                                    border: [true, true, true, true]
                                },
                            ],
                        ]
                    },
                    layout: {
                        paddingTop: function (i, node) { return 2; },
                        paddingBottom: function (i, node) { return 2; },
                        hLineColor: '#4f6228',
                        vLineColor: '#4f6228',
                    }
                },
                '\n',
                {
                    table: {
                        widths: [38, 50, 62, 53, 53, 53, 20, 50, 50],
                        body: students
                    },
                    layout: {
                        hLineColor: '#484848',
                        vLineColor: '#484848',
                        hLineColor: '#484848',
                        vLineColor: '#484848',
                    },

                },
            ],

        styles: {
            header: {
                fontSize: 7,
                bold: true,
            },
            paragraph: {
                fontSize: 9
            },
            subParagraph: {
                fontSize: 10,
                bold:true
            }
        }
        , layout: {
            // paddingLeft: function(i, node) { return 14; },
            // paddingRight: function(i, node) { return 4; },
            paddingTop: function (i, node) { return 0; },
            paddingBottom: function (i, node) { return 0; },
            // fillColor: function (i, node) { return null; }
        },
    };
    // download the PDF
    // pdfMake.createPdf(docDefinition).download('STUDENT_INDUCTION_FORM.pdf');
    // pdfMake.createPdf(docDefinition).getBase64(function(encodedString) {
    //     pdf = encodedString;
    //     $.ajax({
    //         method: "POST",
    //         url: "http://dev.softkodes.com/optimisticfuture/formEmail",
    //         data: {form4: pdf},
    //       }).done(function(data){
    //           console.log(data)
    //       });
    // });
    // Open the pdf in new window
    pdfMake.createPdf(docDefinition).open();
}